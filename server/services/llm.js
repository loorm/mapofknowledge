...
async function generateLootBox(nodeLabel, breadcrumb, locale, userId) {
  ...
  const prompt = `You are a resource scout for KnoBitz, a learning environment. A learner is working through a node on a learning path. Your job is to fill a "Loot Box" — optional extra material that sits alongside the main lesson, in nine fixed categories.
